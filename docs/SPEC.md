...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'agent';
...
Messages must start with the trigger pattern (default: `@agent`):
- `@agent what's the weather?` → ✅ Triggers Claude
- `@Agent help me` → ✅ Triggers (case insensitive)
- `Hey @agent` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @agent what toppings do you recommend?
```
...
| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@agent add group "Family Chat"` | Register a new group |
...
```
User: @agent remind me every Monday at 9am to review the weekly metrics
...
```
User: @agent at 5pm today, send me a summary of today's emails
...
From any group:
- `@agent list my scheduled tasks` - View tasks for this group
- `@agent pause task [id]` - Pause a task
- `@agent resume task [id]` - Resume a paused task
- `@agent cancel task [id]` - Delete a task

From main channel:
- `@agent list all tasks` - View tasks from all groups
- `@agent schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@agent add group "Name"` in main |
