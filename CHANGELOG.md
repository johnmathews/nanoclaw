...
- **Fixed: outbound messages silently lost on Slack and WhatsApp.** The delivery loop treated *any* non-throwing return from a channel adapter as proof of delivery, and several adapter paths returned `undefined`-as-success without actually sending — WhatsApp swallowed disconnect/send errors into an in-memory queue (lost on restart), the registry logged-and-returned when an adapter was offline, and permanent Slack errors (`missing_scope`, `message_not_found`) were marked `failed` and never surfaced. The agent recorded the send in its outbound DB, so it "remembered" sending a message the user never got. Introduced a delivery-error taxonomy (`src/channels/delivery-errors.ts`: `ChannelDisconnectedError`, `PermanentDeliveryError`, `classifyDeliveryError`): adapters now **throw** when they can't send (never return undefined-as-success); offline channels are `deferred` with backoff and re-driven across restarts (new `delivered.status='deferred'` + `attempts`/`next_attempt_at` columns); permanent failures and exhausted transient retries are surfaced to the agent as a context-only (`trigger=0`) notice in `messages_in`. Host-only change — takes effect on host restart; existing session DBs auto-migrate on the next delivery poll.
- **Fixed: WhatsApp replies stalled until the next inbound message.** A regression from the silent-loss fix above: removing WhatsApp's in-memory `outgoingQueue`/`flushOutgoingQueue` also removed the only delivery trigger that fired on socket reconnect. WhatsApp re-opens its socket ~150×/day; a reply composed during a reconnect gap (or against a "zombie" socket — `connection==='open'` but the server stopped acking) was handed to the host's poll-based re-drive, but once the agent's turn ended the session dropped out of the fast 1s poll and the reply waited for the 60s sweep — in practice, until the user sent another message that re-woke the container (the "ask → silence → poke → reply appears instantly" symptom). Three fixes: (1) a new optional `ChannelSetup.onReconnect` hook — WhatsApp fires it on every reconnect and the host immediately re-drives all active sessions (`redriveActiveSessionsNow`), restoring the old flush-on-reconnect non-lossily; (2) sessions with a non-terminal delivery (deferred in backoff, or mid transient-retry) stay on the fast 1s poll even after their container goes idle/stopped (`pendingRedrive`), so a finished-turn reply no longer waits for the sweep; (3) `sock.sendMessage` now has a 15s timeout — on a zombie socket it tears the connection down to force a reconnect and throws `ChannelDisconnectedError` (defer + re-drive) instead of hanging the session's delivery indefinitely. Host-only; takes effect on restart.
- **Slack agents now always get Slack formatting rules.** `slack-formatting` previously shipped only an on-demand `SKILL.md`, so Slack agents carried the always-on `whatsapp-formatting` fragment but never auto-loaded Slack rules — letting mrkdwn footguns through (e.g. an email address hand-wrapped as `ritsya<@gmail.com>`, where `@gmail.com` is mis-parsed as a failed `<@USERID>` mention). Added `container/skills/slack-formatting/instructions.md` (an always-on fragment, mirroring `whatsapp-formatting`) leading with the email/mention gotchas, and documented the email rule in `SKILL.md`. CONTRIBUTING now explains the `instructions.md` (always-on) vs `SKILL.md` (on-demand) load modes for container skills. Takes effect on next container spawn — fragments recompose, no image rebuild.
